...
interface SessionRequestDisplay {
  id: string;
  student_id: string;
  coach_id: string;
...
interface CoachScheduleContentProps {
  coachId: string;
}

export default function CoachScheduleContent({ coachId }: CoachScheduleContentProps) {
  const [currentUser, setCurrentUser] = useState<any>(null);
  const [coach, setCoach] = useState<CoachProfile | null>(null);
  const [availability, setAvailability] = useState<AvailabilitySlot[]>([]);
  const [sessionRequests, setSessionRequests] = useState<SessionRequestDisplay[]>([]);
    ...
    async function loadCoachData() {
      try {
        ...
        await Promise.all([
          ...
          fetchSessionRequests(user.id, coachId)
        ]);
      } catch (error: any) {
  ...
  async function fetchSessionRequests(studentId: string, coachIdParam: string) {
    try {
      ...
      const mappedRequests = requestsData.map(request => ({
        ...
        coach_response: request.coach_response,
        student: {
