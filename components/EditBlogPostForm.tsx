  ...
  useEffect(() => {
    async function loadPost() {
      ...
      try {
        ...
        await Promise.all([
          fetchPost(postId, user.id, profile?.role || ''),
          fetchCategories(),
          fetchTags()
        ]);
      } catch (error: any) {
        ...
        <Card>
          <CardHeader>
            <div className="flex justify-between items-start">
              <div>
                <CardTitle className="text-2xl">{form.watch('title') || post.title}</CardTitle>
                <CardDescription className="mt-2">
                  {form.watch('excerpt') || post.excerpt}
                </CardDescription>
              </div>
              <div className="flex gap-2">
                {form.watch('featured') && (
          ...
          <CardContent>
            <div className="prose max-w-none">
              <div className="whitespace-pre-wrap">
                {form.watch('content') || post.content}
              </div>
            </div>
