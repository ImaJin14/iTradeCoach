...
CREATE TABLE IF NOT EXISTS user_settings (
    ...
    CHECK ((coach_id IS NOT NULL AND student_id IS NULL) OR 
       (coach_id IS NULL AND student_id IS NOT NULL)),
...
BEGIN
    ...
    WITH inserted_students AS (
        ...
        WHERE role = 'student' AND id NOT IN (SELECT student_id FROM student_profiles WHERE student_id IS NOT NULL)
    ...
    WITH inserted_coaches AS (
        ...
        SELECT id FROM profiles 
        WHERE role = 'coach' AND id NOT IN (SELECT coach_id FROM coach_profiles WHERE coach_id IS NOT NULL)
